...
export default function ArmoryPage() {
	...
	const displayStates = useMemo(
		() => getEquipmentDisplayStates(equipment),
		[equipment]
	);
	...
	const filteredDisplayStates = useMemo(() => {
		return displayStates.filter((state) => {
			...
			if (searchQuery) {
				...
				const matchesCategory = state.category.name
					.toLowerCase()
					.includes(query);
				const matchesItems = state.items.some(
					(item) =>
			...
			if (filterStatus === 'maintenance' && state.needsMaintenance === 0)
				return false;
	...
	// Separate primary and auxiliary equipment for visual hierarchy
	const primaryEquipment = useMemo(
		() =>
			filteredDisplayStates.filter((state) =>
				PRIMARY_EQUIPMENT_TYPES.includes(state.category.type)
			),
		[filteredDisplayStates]
	);

	const auxiliaryEquipment = useMemo(
		() =>
			filteredDisplayStates.filter((state) =>
				AUXILIARY_EQUIPMENT_TYPES.includes(state.category.type)
			),
		[filteredDisplayStates]
	);
		...
		<main className="container mx-auto max-w-7xl p-6 space-y-6">
			...
			{filteredDisplayStates.length === 0 ? (
				...
				<div className="space-y-8">
					{/* Primary Equipment - Larger cards */}
					{primaryEquipment.length > 0 && (
						<div>
							<div className="flex items-center gap-2 mb-4">
								<h2 className="text-xl font-semibold">Essential Equipment</h2>
								<Badge variant="secondary" className="text-xs">
									{primaryEquipment.length} categories
								</Badge>
							</div>
							<div
								className={cn(
							...
							>
								{primaryEquipment.map((displayState) => (
									<EquipmentCard
					...
					{/* Auxiliary Equipment - Smaller cards */}
						...
						<div>
							<div className="flex items-center gap-2 mb-4">
								...
								<Badge variant="outline" className="text-xs">
									{auxiliaryEquipment.length} categories
								</Badge>
							</div>
							<div
								className={cn(
							...
							>
								{auxiliaryEquipment.map((displayState) => (
									<EquipmentCard
										key={displayState.category.type}
										displayState={displayState}
										onAddItem={() => handleAddItem(displayState.category.type)}
										...
										className={cn(
											'scale-95 transform',
											viewMode === 'list' && 'max-w-none scale-100'
										)}
									/>
								))}
							</div>
